...
def prep_triplets(triplets, cuda):
    ...
    """
    a, n, d = (
        Variable(triplets["anchor"]),
        Variable(triplets["neighbor"]),
        Variable(triplets["distant"]),
    )
    if cuda:
...
def train_triplet_epoch(
    model, cuda, dataloader, optimizer, epoch, margin=1, l2=0, print_every=100, t0=None
):
    """
    ...
    for idx, triplets in enumerate(dataloader):
        ...
        if (idx + 1) * dataloader.batch_size % print_every == 0:
            ...
            print(
                "Epoch {}: [{}/{} ({:0.0f}%)], Avg loss: {:0.4f}".format(
                    epoch,
